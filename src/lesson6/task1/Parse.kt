...
fun dateStrToDigit(str: String): String {
    ...
    if ((parts[0].toInt() < 1) || (parts[0].toInt() > daysInMonth(mm, parts[2].toInt())))
        return ""
    return String.format("%02d.%02d.%d", parts[0].toInt(), mm, parts[2].toInt())
...
fun bestLongJump(jumps: String): Int {
    ...
    for (i in jumps) {
        if ((i !in '0'..'9') && (i != '-') && (i != '%') && (i != ' ')) return -1
    }
    val parts = jumps.split("-", "%", " ").toMutableList()
    parts.removeIf { it.isEmpty() }
    if (parts.joinToString("") == "") return -1
    return parts.map { it.toInt() }.max()!!
...
fun firstDuplicateIndex(str: String): Int {
    val wordsList = str.split(" ").toMutableList()
    for (i in 1 until wordsList.size) {
        if (wordsList[i].toLowerCase() == wordsList[i - 1].toLowerCase())
            return str.indexOf(" " + wordsList[i - 1] + " " + wordsList[i] + " ") + 1
    }
    return -1
}
